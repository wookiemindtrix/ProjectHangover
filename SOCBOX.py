...
import os
import pickle
from PyQt6.QtWidgets import QApplication, QMainWindow, QPushButton, QHBoxLayout, QVBoxLayout, QLabel, QFileDialog, QTextEdit, QWidget, QInputDialog, QMessageBox
...
class ScriptRunner(QMainWindow):
    ...
    def __init__(self):
      ...
      ( '.( OO )_   _(  OO) ( OO ).-.    ( OO ) ) 
        ...
        self.subtitle_label.setText("Developers: Matt, Eddie, Andre, Nate (M.E.A.N)<br>Fullstack Academy - Cohort 2212-FCB-ET-CYB-PT<br>May 2023<br>")
    ...
    def remove_script(self):
        ...
        button_name, ok = QInputDialog.getItem(self, "Remove Tool", "Select the tool to remove:", list(script_buttons.keys()), 0, False)
        print(f"button_name: {button_name}, ok: {ok}")
        if ok and button_name:
            confirm_options = ["Yes", "No"]
            confirm_choice, _ = QInputDialog.getItem(self, "Confirm Removal", "Are you sure you want to remove this tool?", confirm_options, 0, False)
        if confirm_choice == "Yes":
            print(f"Removing {button_name}")
            self.remove_script_button(button_name)
            self.reload_script_buttons()
        else:
            print("No button_name or ok is False")  # This line will print if ok is False or button_name is None or an empty string
    ...
    def load_script_buttons(self):
        ...
        current_buttons = [self.layout.itemAt(i).widget().text() for i in range(self.layout.count()) if isinstance(self.layout.itemAt(i).widget(), QPushButton)]
